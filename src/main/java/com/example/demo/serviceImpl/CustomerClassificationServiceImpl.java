...
//		
//		//we need to map all the objetcts that are there in the list from dto to object
//		
//		for(custDetailDTO t:dto.getCustomerDetails()) {
//			CustomerDetail temp;
//			CustomerClassificationType classificationType=classificationRepo.findById(t.getClassificationId()).orElseThrow(()-> new RuntimeException("ID not found"));
//			
//			CustomerIdentification cIdentification=new CustomerIdentification(t.getIdentification().getCustomerIdentifId(), t.getIdentification().getCustIdentificationtype(), temp, t.getIdentification().getCustIdentificationItem(), t.getIdentification().getEffectiveDate()); 
//			
//			List<CustomerName> allcustomerNames=new ArrayList<>();
//			for(custNameDTO nameDTO:t.getCustomerNames()) {
//				CustomerName tempCustomerName=new CustomerName(nameDTO.getCustNameId(), temp, classificationType, nameDTO.getCustomerNameType(), nameDTO.getCustomerNameValue(), nameDTO.getEffectiveDate());
...
//				CustomerProofofId proofs=new CustomerProofofId(proofOfIdDTO.getCustomerProofId(), temp, proofOfIdDTO.getProofofIdType(), proofOfIdDTO.getProofofIdValue(), proofOfIdDTO.getEffectivDate(), proofOfIdDTO.getStartDate(), proofOfIdDTO.getEndDate());
...
//				CustomerContactInformation cInformation=new CustomerContactInformation(infoDTO.getCustContactId(), temp, infoDTO.getCustomerContactType(), infoDTO.getCustomerContactValue(), infoDTO.getEffectiveDate(), infoDTO.getStartDate(), infoDTO.getEndDate());
...
//				CustomerAddress cAddress=new CustomerAddress(addressDTO.get, temp, classificationType, addressDTO.getCustomerAddressType(),addressDTO.getCustomerAddressValue(),addressDTO.getEffectiveDate());
//				allAddresses.add(cAddress);
//			}
//			
//			
//			
//			temp=new CustomerDetail(t.getCustId(), t.getCustFullname(), t.getCustGender(), t.getCustDate(), t.getCustPrefLanguage(), t.getCustStatus(), t.getCustCountry(),classificationType ,cIdentification,allcustomerNames, allProofofIds, allContactInformations, allAddresses);
...
//		return customerDetailRepo.findByClassificationType_CustomerClassificationId(classifId);
//		
//		
//	}
//	
