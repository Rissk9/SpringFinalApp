...
////	    		temp=addressRepo.findById(n.getCustAddressId()).orElseThrow(()->new RuntimeException("Customer address id not found"));
////
////	    		temp.setCustomerAddressType(n.getAddressType());
////	    		temp.setCustomerAddressValue(n.getAddressValue());
////	    		temp.setEffectiveDate(n.getEffectiveDate());
...
//	}
	...
	public ResponseEntity<?> deleteCustomer(Long custId) {
		...
		CustomerDetail customerDetail=repo.findById(custId).orElseThrow(()-> new RuntimeException("The id is not found"));
		
		if(customerDetail.getCrudval()=='D')
			return new ResponseEntity<>("Customer already deleted ",HttpStatus.BAD_REQUEST);
